...
export class HomeService {
  static async getHomeDashboard(userId: string, organizationId: string): Promise<IHomeDashboardData> {
    const [userDoc, orgDoc, channelsCount, totalPostsCount, scheduledPosts, recentCommentsDoc, customTemplates] =
      await Promise.all([
        User.findById(userId).select('name email avatar role').lean(),
        Organization.findById(organizationId).select('name plan').lean(),
        Channel.countDocuments({ organizationId, isConnected: true }),
        Post.countDocuments({ organizationId }),
        Post.find({
          organizationId,
    ...
    const totalComments = await Comment.countDocuments({ organizationId });
    const repliedComments = await Comment.countDocuments({ organizationId, repliedContent: { $exists: true, $ne: '' } });
    ...
    const upcomingPosts = scheduledPosts.map((p: any) => ({
      id: p._id.toString(),
      content: p.content,
      mediaUrls: p.mediaUrls,
      scheduledAt: p.scheduledAt,
      status: p.status,
      channels: (p.channelIds || []).map((ch: any) => ({
    ...
    const recentComments = recentCommentsDoc.map((c: any) => ({
      id: c._id.toString(),
    ...
    const templates =
      customTemplates && customTemplates.length >= 4
