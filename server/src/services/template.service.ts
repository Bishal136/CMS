...
export const SEEDED_DISCOVER_TEMPLATES = [
...
export class TemplateService {
  private static async ensureSeeded() {
    const count = await Template.countDocuments({ isDiscoverable: true });
    if (count === 0) {
      try {
        await Template.insertMany(SEEDED_DISCOVER_TEMPLATES);
      } catch {
  ...
  static async listTemplates(
    ...
    filter?: { category?: string; isPersonal?: boolean; search?: string }
  ) {
